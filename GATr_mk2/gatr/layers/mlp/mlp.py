...
class GeoMLP(nn.Module):
    ...
    def __init__(
        ...
        layers.append(EquiLinear(
            in_mv_channels=config.mv_channels[0],
            out_mv_channels=config.mv_channels[0],
            in_s_channels=s_channels[0],
            out_s_channels=s_channels[0],
        ))
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            if i == 0:
                mv, s = layer(mv, scalars = s) # normalization
            elif i == 2:
                mv, s = layer(mv, scalars=s, reference_mv=reference_mv) # bilinear
            else:
