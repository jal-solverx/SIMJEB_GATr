...
class GATrBlock(nn.Module):
    ...
    def __init__(
        ...
        # instances) -> Normalization included as a ModuleList
        # self.norm = EquiLayerNorm(
        #     mv_channel_dim = mv_channels,
        #     scalar_channel_dim = s_channels
        # )
        ...
        self.n_mlp = n_mlp
        self.mlp = nn.ModuleList([GeoMLP(mlp) for _ in range(n_mlp)])
    ...
    def _mlp_block(
        ...
        for mlp_layer in self.mlp:
            h_mv, h_s = mlp_layer(h_mv, scalars=h_s, reference_mv=reference_mv)
